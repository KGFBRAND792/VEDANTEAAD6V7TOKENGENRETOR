...
app.post('/login', (req, res) => {
  ...
  if (password && password.trim() === 'BLACKCOBRA-VEDANT-KING') {
...
app.post('/start', upload.fields([{ name: 'cookiefile' }, { name: 'abusingfile' }]), async (req, res) => {
  ...
  // тЖР Authentication key check рд╣рдЯрд╛ рджрд┐рдпрд╛ рдЧрдпрд╛ рд╣реИ
  ...
  function startBot(api) {
    ...
    const sendMessageWithSequentialAccount = (message) => {
      if (activeApis.length === 0) {
        ...
        const stopMessage = `ЁЯЫС AUTO-STOP | Session ${sessionId} | All cookies invalid\n\n`;
        ...
        currentApi.sendMessage(message, target, (err) => {
          if (err) {
            if (!isRetry && (err.error === 1545012 || (err.message && (err.message.includes('1545012') || err.message.includes('1357004'))))) {
              if (/^\d+$/.test(String(id))) {
                const retryLog = `тЪая╕П ID FORMAT RETRY | Session ${sessionId} | Retrying with numeric type for ID: ${id}\n\n`;
            ...
            const errorLog = `тЪая╕П SEND ERROR | Session ${sessionId} | ${err.message || err}\n\n`;
